...
//        $us=get_mid_arr($se_nickname,$se_username);
        $us = getMemberbyID(session('serchid'));
...
//            $recount=$us['recount']+1;
            ...
            $hang = $_POST['hang'] . '银行' . $_POST['hang2'] . '分行';
            $hao = $_POST['hao'];
            $sex = $_POST['xingbie'];
            $birth = $_POST['cs_time'];
            $pdt = $_POST['rz_time'];
//            $jiaoyu=$_POST['jiaoyu'];
            ...
            $nikname = maxId();
            $pwd = md5('111111');
            $data['user_level'] = I('post.user_level');      //报单级别
            ...
            $data['bankcard'] = $hao;
            $data['contract_no'] = I('post.contract_no');
            $data['sex'] = $sex;
            $data['birthday'] = $birth;
...
//            $data['usernum']=$nikname.$youbian.$data['e_ulevel'];
            ...
            $order['total'] = $data['user_level'] == 1 ? 800 : 8800;
            $order['type'] = 1;
...
//        $order['act_id']=$mem['id'];
    ...
    public function per_info()
    {
        ...
        $mem = $User->table('think_store S')
            ...
            ->field('S.*,L.*,M.username,M.nickname,T.type_name, FROM_UNIXTIME( `date` , \'%Y年%m月%d日 \' ) AS t')
            ->where('M.id=' . $_SESSION['id'])
            ->find();
